...
from db_connection import cur, date


from PhoneBook.About_pg import About_us
from PhoneBook.addPeopple import Add_Friend
from PhoneBook.mypeople import MyPeople
...
class Application(object):
    def __init__(self, master):
        ...
        self.db_btn = Button(self.bottomFrame, text='Connect DB', width=15, height=1, command=self.db_status)
    ...
    def db_status(self):
        ...
        cur_time = cur.execute(stmt)
        if cur_time:
            print('db_connected')
            self.db_btn = Button(self.bottomFrame, text='DB Connected', width=15, height=1, command=self.db_status)
            self.db_btn.place(x=270, y=325)
